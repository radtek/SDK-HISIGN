 ...
 * @create time：2012-5-22  18:23:56
 * @version 1.0
 */
...
public class SafeProperties extends Properties {
    ...
    class PropertiesContext {
        ...
        public void putOrUpdate(String key, String value, String line) {
            int index =  this.getProperty(key);
            if(index == -1){
                PropertyEntry pe = new PropertyEntry(key, value, line);
                commentOrEntrys.add(pe);
            }else{
                PropertyEntry pe = (PropertyEntry)commentOrEntrys.get(index);
                pe.setValue(value);
                pe.setLine(line);
                commentOrEntrys.set(index, pe);
            }
        }
