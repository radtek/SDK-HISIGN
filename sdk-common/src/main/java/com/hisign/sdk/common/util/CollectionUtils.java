...
public abstract class CollectionUtils {
	...
	/**
	 * 将对象类型Map拷贝到字符串类型Map中
	 * @param srcObjMap 源对象Map
	 * @param destStrMap 目的字符串Map
	 * @param isConverSame 如果存在相同的key,是否覆盖
	 */
	public static void putObjMapToStrMap(final Map srcObjMap,Map<String,String> destStrMap,boolean isConverSame){
		if(srcObjMap == null || srcObjMap.size() <= 0  || destStrMap == null){
			return;
		}
		
		Iterator<Map.Entry> it = srcObjMap.entrySet().iterator();
		while(it.hasNext()){
			...
			if(destStrMap.containsKey(key)){ //存在相同的key
				if(isConverSame == true){
					destStrMap.put(key, value);
				}
			}else{
				destStrMap.put(key, value);
			}
