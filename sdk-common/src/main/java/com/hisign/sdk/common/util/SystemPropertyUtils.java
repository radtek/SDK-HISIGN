...
public abstract class SystemPropertyUtils {
	...
	public static Properties resolvePropertiesPlaceholders(Properties sysProp) {
	    Properties newProp = new Properties();
	    Iterator it = sysProp.entrySet().iterator();
	    while(it.hasNext()){
	        Map.Entry<String,String> entry = (Map.Entry<String, String>)it.next();
	        String key = entry.getKey();
	        String value = entry.getValue();
	        if(value.indexOf(PLACEHOLDER_PREFIX) != -1){
	            value = resolvePlaceholders(value,sysProp);
	        }
	 ...
	 * @return
	 */
	public static String resolvePlaceholders(String text,Properties sysProp) {
	    StringBuffer buf = new StringBuffer(text);
        ...
        while (startIndex != -1) {
            ...
            if (endIndex != -1) {
                ...
                int nextIndex = endIndex + PLACEHOLDER_SUFFIX.length();
                try {
                    String propVal = sysProp.getProperty(placeholder);
                    if (propVal != null) {
                        buf.replace(startIndex, endIndex + PLACEHOLDER_SUFFIX.length(), propVal);
                        nextIndex = startIndex + propVal.length();
                    }
	...
	public static void main(String []args){
		System.out.println(resolvePlaceholders("*_dog.xml"));
	}
}
